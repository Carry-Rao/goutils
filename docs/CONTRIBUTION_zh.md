...
感谢您考虑为 Goutils 贡献代码。本文档详细介绍了项目的提交规范、代码风格和工作流程，帮助您快速上手。
...
| `<操作类型>` | 是 | 操作类型（见下方表格） |
| `, Module:` | 否 | 影响的模块（如 `database`、`log`、`docs`） |
| `: <描述>` | 否 | 变更的简要描述 |
...
本项目使用 **Go** 语言，遵循 [官方 Go 代码评审建议](https://go.dev/wiki/CodeReviewComments)。此外，CI 流水线会进行以下静态分析检查：

### 函数长度

- 函数长度必须在 **4 到 50 行**之间（含）。
- 函数少于 4 行会触发 CI 警告。
- 函数超过 50 行会触发 CI 警告。

### 错误处理

- **禁止**使用空标识符（`_`）忽略返回的错误，除非返回值来自 `fmt` 包。
- 避免的错误处理方式：
  ```go
  // 错误：忽略了非 fmt 的错误
  _ = db.Close()
...
- 允许的错误处理方式：
  ```go
  // 正确：fmt.Fprint 返回 (int, error)，可安全忽略
